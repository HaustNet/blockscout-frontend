...
const ChartArea = ({ id, xScale, yScale, color, data, disableAnimation, ...props }: Props) => {
  ...
  const gradientStopColor = useToken('colors', useColorModeValue('bronze.900', 'blackAlpha.100'));
  const defaultGradient = {
    startColor: useToken('colors', useColorModeValue('bronzeAlpha.250', 'blue.400')),
    stopColor: useToken('colors', transparentize(useColorModeValue('bronzeAlpha.50', 'blue.400'), 0)(theme)),
  };
